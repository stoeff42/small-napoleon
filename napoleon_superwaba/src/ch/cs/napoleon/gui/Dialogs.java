...
public class Dialogs {
    ...
    public static String askName(Window window, String defaultName) {
        showMessageBox(window, "Finished", "Got it!", Control.LEFT);

        InputDialog inputDialog = new InputDialog("High score!",
    ...
    public static void showAbout(Window window) {
        showMessageBox(window, "About",
            "Little Napoleon|by Christoph Schilling", Control.CENTER);
    }
     ...
     */
    public static void showScores(Window window, final ScoreEntries scoreEntries) {
        ...
        Container scoreContainer = getScoreContainer(dialog, scoreEntries);
        dialog.setRect(Control.CENTER, Control.CENTER,
            ...
            scoreContainer.getSize().y + okButton.getPreferredHeight() + 1);
    ...
    private static Container getScoreContainer(Window window,
        ...
        for (int i = 0; i < columnCount; i++) {
            ...
            switch (i) {
            case (0):
                label = new Label((String) ScoreEntry.getHeaderNames()[i],
                        Control.LEFT);
        ...
        for (int i = 0; i < rowCount; i++) {
            ...
            Label label = new Label(entry.getName(), Control.LEFT);
            updateWidthsAndHeights(widths, heights, 0, i + 1, label);
            scoreContainer.add(label);
            label = new Label(Convert.toString(entry.getNrOfMoves()),
                    Control.RIGHT);
            updateWidthsAndHeights(widths, heights, 1, i + 1, label);
            scoreContainer.add(label);
            label = new Label(Dialogs.makeDurationString(entry.getTime()),
                    Control.RIGHT);
        ...
        for (int j = 0; j < (rowCount + 1); j++) {
            ...
            for (int i = 0; i < columnCount; i++) {
                controls[controls.length - (i + (j * columnCount)) - 1].setRect(width,
                    height, widths[i], heights[j]);
                width += (widths[i] + 1);
            }
     ...
     * @param scoreEntries TODO: javadoc
     */
    protected static void clearScores(Window window, ScoreEntries scoreEntries) {
        if (scoreEntries.getNrOfEntries() == 0) {
            showNoScores(window);
        }

        MessageBox messageBox = new MessageBox("Delete scores",
                wrap("Are you sure you want to delete the scores?", window),
    ...
    private static void showMessageBox(Window window, String title,
        ...
        MessageBox messageBox = new MessageBox(title, wrap(text, window));
    ...
    private static void showNoScores(Window window) {
        showMessageBox(window, "Scores", "No scores yet!", Control.LEFT);
    }
     ...
     */
    private static void updateWidthsAndHeights(int[] widths, int[] heights,
        int i, int j, Label label) {
        widths[i] = Math.max(widths[i], label.getPreferredWidth());
        heights[j] = Math.max(heights[j], label.getPreferredHeight());
    }
    ...
    private static String wrap(String text, Window window) {
        FontMetrics fontMetrics = window.getFontMetrics(window.getFont());
        ...
        for (int i = 0; i < paragraphs.length; i++) {
            ...
            wrap.append(Convert.insertLineBreak(Settings.screenWidth - 6, '|',
